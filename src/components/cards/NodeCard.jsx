...
import { Radio, MapPin, Check, AlertTriangle, Clock, Calendar, Activity, Edit2, Trash2 } from "lucide-react";
...
export default function NodeCard({ node }) {
  ...
  const handleUpdate = () => {
    updateNode({ ...form, maop: Number(form.maop), pressure: Number(form.pressure) });
    setEditing(false);
  };

  const isCritical = ratio >= settings.warningThreshold;
  ...
  return (
    <div className="node-card-wrap" style={{ borderColor: isCritical ? '#fecaca' : '#f3f4f6', boxShadow: isCritical ? '0 1px 2px 0 rgba(254, 202, 202, 0.5)' : undefined }}>
      <div className="node-card-header">
        <div className="node-card-header-left">
          <div className="node-card-icon-box" style={{ backgroundColor: isCritical ? '#fef2f2' : '#eff6ff', color: isCritical ? '#ef4444' : '#3b82f6' }}>
      ...
      <div className="node-card-stats">
        <div>
          ...
          <p className="node-card-stat-val" style={{ color: isCritical ? '#dc2626' : undefined }}>{node.pressure.toFixed(1)} PSI</p>
        </div>
        <div>
      ...
      <div className="node-card-footer">
        <div className="node-card-footer-item">
          ...
          <div className="node-card-footer-val">42 mins ago</div>
        </div>
        ...
        <div className="node-card-footer-item">
          ...
          <div className="node-card-footer-val">{node.pipeAge || Math.floor(Math.random() * 10) + 1} years</div>
        </div>
        ...
        <div className="node-card-footer-item">
          ...
          <div className="node-card-footer-val">Active</div>
        </div>
      </div>
    </div>
  );
